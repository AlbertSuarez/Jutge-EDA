...
void reines (Matriu& r, vector<bool>& col, vector<bool>& dia1, vector<bool>& dia2, int n, int f, bool& trobat) {
   if (f == n) {
      trobat = true;
      escriu(r, n);
   }
 	else {
      for (int c = 0; c < n and not trobat; ++c) {
         int d1 = diag1(n, f, c);
         int d2 = diag2(f, c);
         if (not col[c] and not dia1[d1] and not dia2[d2]) {
            ...
            dia1[d1] = true;
            dia2[d2] = true;
            reines(r, col, dia1, dia2, n, f+1, trobat);
            r[f][c] = '.';
            col[c] = false;
            dia1[d1] = false;
            dia2[d2] = false;
...
int main () {
   ...
   bool trobat = false;
   reines(reinas, col, dia1, dia2, n, 0, trobat);
